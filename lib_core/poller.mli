...
(** GitHub poller: translates PR world state into patch state updates.
    ...
    The merge-conflict rule excludes merge-queue entries. While a PR sits in a
    merge queue its head branch is push-locked (GH006 [Merge_queue_locked]), so
    a [Merge_conflict] op could only no-op locally and bounce off the lock,
    walking [conflict_noop_count] toward spurious intervention. The
    [has-conflict'] {e flag} rule stays ungated — it purely mirrors GitHub
    state, and a genuine conflict makes GitHub eject the PR from the queue,
    after which the next poll enqueues the op normally. *)
